...
export default function Sidebar({
  ...
  const filteredMaterials = useMemo(() => {
    ...
    return materials.filter((m) => {
      if (filters.collectionId != null && m.collectionId !== filters.collectionId) {
        return false;
      }
  ...
  const handleRequestAddSubcollection = useCallback((folder) => {
    handleExpandFolder(folder.id);
    setCreatingInFolderId(folder.id);
  }, [handleExpandFolder]);

  const handleSubmitSubcollection = useCallback((parentId, name) => {
  ...
  const handleSubmitAddModal = useCallback(async (data) => {
  ...
  const handleSoftDelete = useCallback((item, itemType) => {
  ...
  const buildMenuItems = useCallback((item, section = 'tree') => {
  ...
  const handleContextMenu = useCallback((e, item, section = 'tree') => {
    const items = buildMenuItems(item, section);
    setCtxMenu({ x: e.clientX, y: e.clientY, items });
  }, [buildMenuItems]);

  const handleKebabMenu = useCallback((item, anchorEl, section = 'tree') => {
    const items = buildMenuItems(item, section);
    if (!anchorEl) return;
  ...
  const handleRenameSubmit = useCallback((id, name, type) => {
  ...
  const handleDragEnd = useCallback((event) => {
    ...
    const targetParentIdNum = targetParentId == null ? null : Number(targetParentId);
    ...
    if (itemType === 'folder') {
      ...
      const currentParentNum = currentParent == null ? null : Number(currentParent);
    ...
    showToast({
      message: itemType === 'folder' ? 'Коллекция перемещена' : 'Материал перемещён',
      type: 'success',
      duration: 2000,
    });
