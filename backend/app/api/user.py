...
async def update_me(body: UserUpdate, db: DB, user: CurrentUser) -> User:
...
async def remove_avatar(db: DB, user: CurrentUser) -> User:
    """Сброс аватара (файл удаляется, в БД avatar_path = NULL)."""
    return await remove_user_avatar(db, user)


@router.delete("/me", status_code=status.HTTP_204_NO_CONTENT)
async def delete_me(response: Response, db: DB, user: CurrentUser) -> None:
